...
public class EnemyController : MonoBehaviour
{
    ...
    [SerializeField]  float RangeDetection = 50f;
    private float pathUpdateSeconds = 0.5f;
    
    [Header ("Physics")]
    [SerializeField] float velocity = 200f;
    private float netWaypointDistance = 3f;
    private float jumpNodeHeightRequeriment = 0.5f;
    ...
    private void PathFollow()
    {
        ...
        if (jumpEnabled && isGrounded)
        {
            ...
            {
                rb.AddForce(Vector2.up * velocity * jumpModifier);
    ...
    {
        return  Vector2.Distance(transform.position, target.transform.position) < RangeDetection;
    }
